<script>
	...
	listenFirebaseKey(dbLogsArray, (dbLogsArrayRef) => {
		dbLogsArrayRef.on('value', (snap) => {
			...
			logsArray = snap.val();
			if (logsdiv) logsdiv.scrollTo(0, logsdiv.scrollHeight);
		});
	});
	...
	function checkWord(word) {
		...
		updateLeaderBoard({ gameSessionId, winningTeamOnlinePlayersId })
			.then(() => {
	...
	{:else}
				...
				{:else}
					...
					<div class="loosing-btn">
						{#if lastWordSelected?.color === 'Black'}
							Black word selected
						{:else if lastWordSelected?.selectorTeam === team}
			...
			<div class="playerDetails">
				{#if isSpymaster}
					<div class="spymasterBox" on:click={changeView}>
						<div class="spymaster" style="color : {textColorOfSpymaster}">SPYMASTER</div>
						<div class="player" style="color : {textColorOfPlayer}">PLAYER</div>
				...
				{:else}
					<div class="player-box">
						<img
							...
							alt="Your Spymaster"
							style="border : 3px solid {profile_picture_border_color}"
						/>
		...
		<div class="container">
			<div class="word-matrix" style="border : 6px solid {tableBorderColor};">
				...
				{/each}
				{#if showSelectedInfo}
					<span class={selectedInfoType === 1 ? 'Word-show' : 'Word-hide'} style="background-color : {tableBorderMap[selectedInfoType]};"
						>&#x1F60D Correct word Selected</span
					>
					<span class={selectedInfoType === 2 ? 'Word-show' : 'Word-hide'} style="background-color : {tableBorderMap[selectedInfoType]};"
						>&#128532 Opponent selects correct word</span
					>

					<span class={selectedInfoType === 3 ? 'Word-show' : 'Word-hide'} style="background-color : {tableBorderMap[selectedInfoType]};">
						&#128532 Grey word Selected</span
					>
					<span class={selectedInfoType === 4 ? 'Word-show' : 'Word-hide'} style="background-color : {tableBorderMap[selectedInfoType]};"
						>&#x1F60D Opponent selects grey word
					</span>

					<span class={selectedInfoType === 5 ? 'Word-show' : 'Word-hide'} style="background-color : {tableBorderMap[selectedInfoType]};">
						&#128532 Opponent word Selected</span
					>
					<span class={selectedInfoType === 6 ? 'Word-show' : 'Word-hide'} style="background-color : {tableBorderMap[selectedInfoType]};"
						>&#x1F60D Opponent selects your word
					</span>

					<span class={selectedInfoType === 7 ? 'Word-show' : 'Word-hide'} style="background-color : {tableBorderMap[selectedInfoType]};">
						&#128532 Black word Selected</span
					>
					<span class={selectedInfoType === 8 ? 'Word-show' : 'Word-hide'} style="background-color : {tableBorderMap[selectedInfoType]};"
						>&#x1F60D Opponent selects black word</span
					>
				...
				{#if isSpymaster && team === turn}
					{#if !clue || clue.clueSenderTeam !== turn}
						<form class="form-container" on:submit={giveClue}>
							...
							<button class="give-clue-btn" type="submit"> Give Clue </button>
						</form>
						<div class="sendClueMsg">Send clue to your team</div>
					{/if}
					{#if allUsersOnlineStatus[user.id] === false}
						<div class="noOnlinePlayer">You are offline, please check your connection and refresh the page.</div>
					{/if}
					...
					{#if (team === 'Red' && !redTeamPlayersCount) || (team === 'Blue' && !blueTeamPlayersCount)}
						<div class="noOnlinePlayer">No online player in your team. To continue the game ask someone to join.</div>
					{/if}
				...
				{:else}
					...
					{:else if !isSpymaster && team === turn}
						{#if team === 'Red' && !redTeam_has_Spymaster}
							<div class="redSpymasterDisappear">Your Spymaster is offline. To continue the game ask him to join.</div>
						{:else if team === 'Blue' && !blueTeam_has_Spymaster}
							<div class="blueSpymasterDisappear">Your Spymaster is offline. To continue the game ask him to join.</div>
						{/if}
						<div class="clueWaiting">Waiting for clue ...</div>
					{:else if team !== turn}
						{#if turn === 'Red' && !redTeam_has_Spymaster}
							<div class="redSpymasterDisappear">Red Spymaster is offline. To continue the game ask him to join.</div>
						{:else if turn === 'Blue' && !blueTeam_has_Spymaster}
							<div class="blueSpymasterDisappear">Blue Spymaster is offline. To continue the game ask him to join.</div>
						{/if}
					{/if}
					{#if showSelectedInfo}
						<div class="postWordClickMsgBox">{postWordClickMsg}</div>
		...
		<div class="blueTeam_List">
			<div class="blue_heading">Blue Team</div>
		...
		<div class="redTeam_List">
			<div class="red_heading">Red Team</div>
			<div class="userContainer">
				<div class="users">
...
<style>
	...
	* {
	...
	.redSpymasterDisappear,
	.blueSpymasterDisappear {
		color: rgba(255, 255, 255, 1);
		font-family: 'Manrope', sans-serif;
		font-weight: 700;
		font-size: 14px;
